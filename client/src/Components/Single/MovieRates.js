...
function MovieRates({ movie }) {
  ...
  const deleteReviewHandler = (id) => {
    if (window.confirm("Do you want to delete your comment?")) {
      dispatch(deleteReviewMovieAction(id));
    }
  };
  ...
  return (
    <div className="my-12">
      ...
      <div className="mt-5 xl:grid flex-colo grid-cols-5 gap-12 bg-dry xs:p-10 py-10 px-5 sm:p-20 rounded">
        {/* Write review */}
        <form
              ...
              movie?.reviews?.map((review) => (
                ...
                >
                  ...
                  {review?.userId === userInfo?._id ? (
                    <div className="col-span-1 flex-rows text-xs mr-0">
                      <button
                        onClick={() => deleteReviewHandler(movie?._id)}
