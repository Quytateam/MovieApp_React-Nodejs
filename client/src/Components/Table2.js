...
import { DateFormat, TimeFormat, shortUppercaseId } from "./Notfications/Empty";
    ...
    <tr>
      ...
      {users ? (
        <>
          ...
          <td className={`${Text}`}>{TimeFormat(data?.createdAt)}</td>
          <td className={`${Text}`}>{data?.fullName}</td>
          <td className={`${Text}`}>{data?.email}</td>
          <td className={`${Text}`}>{data?.isAdmin ? "Admin" : "User"}</td>
          <td className={`${Text} float-right flex-rows gap-2 mt-3`}>
            {!data?.isAdmin && (
              <button
...
function Table2({ data, users, OnEditFunction, onDeleteFunction }) {
  return (
    <div className="overflow-x-hidden overflow-hidden relative w-full">
